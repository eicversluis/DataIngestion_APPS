            ...
            using (SqlConnection stagingConnection = new SqlConnection(stagingDbConnectionString))
            {
                ...
                while (true)
                {
                    ...
                    const string updateQueryIsFaulty =
                        "UPDATE HeartRateData SET IsFaulty = CASE WHEN SensorId IS NULL THEN 1 WHEN HeartRateBPM IS NULL THEN 1 WHEN HeartRateBPM < 50 THEN 1 WHEN HeartRateBPM > 180 THEN 1 WHEN EnterTime IS NULL THEN 1 ELSE 0 END WHERE IsProcessed = 0";
                    try
                    {
